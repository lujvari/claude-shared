...
- **Exposed:** your passed workspaces are read-write; short-lived AWS SSO bearer tokens (`~/.aws/sso/cache`) and the glab config token are readable inside the container; `gh`/`GITLAB_TOKEN`/`AZURE_DEVOPS_EXT_PAT` env vars are readable; under `--gh`/`--glab`/`--ado` the matching token is also written to `/etc/gitconfig` as a `url.<host>.insteadOf` rewrite (same blast radius as the env var, different surface — see [Private git module fetch](#private-git-module-fetch)); full outbound network.
