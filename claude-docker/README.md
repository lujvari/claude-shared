...
This is the one credential flag whose op-ref is **defaulted** rather than configured. Every other flag names a class of service (*a* GitLab, *a* Cloudflare account) and so has to ask which one; `--mepdb` names a single database, and an env var the user must set to exactly one value is ceremony, not configuration. Override it the same way as the others when you need to:
...
- **Exposed:** your passed workspaces are read-write (except this launcher's own directory when swept in, read-only unless `--tools` — see Protected above); short-lived AWS SSO bearer tokens (`~/.aws/sso/cache`) and the glab config token are readable inside the container; `gh`/`GITLAB_TOKEN`/`AZURE_DEVOPS_EXT_PAT`/`JIRA_API_TOKEN`/`SUPABASE_ACCESS_TOKEN`/`CLOUDFLARE_API_TOKEN`/`POSTGRES_TEST_URL` env vars are readable (the last of those is a DSN, so it carries a database password rather than a revocable token — see [myEntryPoint test Postgres](#myentrypoint-test-postgres---mepdb)); under `--gh`/`--glab`/`--ado` the matching token is also reachable by in-container `git` via a per-host credential helper in `/etc/gitconfig` that reads the env var at auth time (same blast radius as the env var; unlike the prior `insteadOf` approach the secret is NOT written to `/etc/gitconfig` and does NOT appear in git URL output — see [Private git module fetch](#private-git-module-fetch)); `--jira` only forwards as env vars (no `/etc/gitconfig` write); full outbound network.
