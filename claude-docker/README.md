...
- **Exposed:** your passed workspaces are read-write (except this launcher's own directory when swept in, read-only unless `--tools` — see Protected above); short-lived AWS SSO bearer tokens (`~/.aws/sso/cache`) and the glab config token are readable inside the container; `gh`/`GITLAB_TOKEN`/`AZURE_DEVOPS_EXT_PAT`/`JIRA_API_TOKEN`/`SUPABASE_ACCESS_TOKEN` env vars are readable; under `--gh`/`--glab`/`--ado` the matching token is also reachable by in-container `git` via a per-host credential helper in `/etc/gitconfig` that reads the env var at auth time (same blast radius as the env var; unlike the prior `insteadOf` approach the secret is NOT written to `/etc/gitconfig` and does NOT appear in git URL output — see [Private git module fetch](#private-git-module-fetch)); `--jira` only forwards as env vars (no `/etc/gitconfig` write); full outbound network.
