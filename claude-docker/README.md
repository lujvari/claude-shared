...
| `--glab`     | Mount the platform-appropriate `glab-cli` config dir (:ro) and forward `GITLAB_TOKEN`. Also unmasks any in-container `glab auth login` state; without the flag, `/root/.config/glab-cli/` is hidden by a tmpfs overlay. 1Password fallback: when `GITLAB_TOKEN` is unset on the host and `CLAUDE_DOCKER_GITLAB_OP_REF` is an `op://` reference (e.g. `op://claude-docker/gitlab-pat/credential`), `run.sh` runs `op read` to derive the token. Setting the ref makes 1Password **authoritative**: the glab-config-parse fallback is skipped, so an on-disk `glab` login can't shadow it. |
...
For each opted-in flag, when the host env var (`GH_TOKEN`/`GITHUB_TOKEN`, `GITLAB_TOKEN`, `AZURE_DEVOPS_EXT_PAT`) is unset, `run.sh` runs `op read` on the host and forwards the resulting token into the container. **Setting the op-ref is authoritative for `--gh`/`--glab`: the `gh auth token` / glab-config-parse fallbacks are skipped when the ref is present**, so a stray host login can't shadow the 1Password value (this is what prevents the "multiple logins" drift). Host env vars still win over the op-ref when both are set (an explicit override / escape hatch).
