...
| `--tools`       | Mount a workspace's top-level `tools/` subdir read-**write**. By default that subdir is overlaid **read-only** so a broad mount (e.g. all of `C:\dev`) can't let an in-container `--yolo` process rewrite host tooling — including this launcher — and have the host execute it next start. See [Threat model](#threat-model). Pass `--tools` only when you knowingly need to edit tooling from inside the container. |
...
- **Protected:** host filesystem outside your passed workspaces, host `~/.aws/credentials` (long-lived keys), host AWS/glab config dirs are read-only from inside (container can't persist changes back). A top-level `tools/` subdir of any workspace is overlaid read-only by default (kernel-enforced — root-in-container can't write through it), so a `--yolo` container can't rewrite the host launcher and have it run on the next start. `--tools` opts that subdir back into read-write when you knowingly need to edit tooling.
- **Exposed:** your passed workspaces are read-write (except a top-level `tools/` subdir, read-only unless `--tools` — see Protected above); short-lived AWS SSO bearer tokens (`~/.aws/sso/cache`) and the glab config token are readable inside the container; `gh`/`GITLAB_TOKEN`/`AZURE_DEVOPS_EXT_PAT`/`JIRA_API_TOKEN` env vars are readable; under `--gh`/`--glab`/`--ado` the matching token is also written to `/etc/gitconfig` as a `url.<host>.insteadOf` rewrite (same blast radius as the env var, different surface — see [Private git module fetch](#private-git-module-fetch)); `--jira` only forwards as env vars (no `/etc/gitconfig` write); full outbound network.
