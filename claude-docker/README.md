...
| Flag         | Effect |
|--------------|--------|
| `--aws`      | Mount `~/.aws/config` + `~/.aws/sso` (:ro) and forward `AWS_PROFILE`/`AWS_REGION`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_SESSION_TOKEN`. 1Password fallback: when `AWS_ACCESS_KEY_ID` is unset and `CLAUDE_DOCKER_AWS_OP_REF` points at a 1P item (e.g. `op://claude-docker/aegon-aws`), `run.sh` reads canonical sub-fields `access_key_id`, `secret_access_key` (both required), and `session_token` / `region` (optional) via `op read` and forwards them as the matching `AWS_*` env vars. Lets a host with no `~/.aws/` config still authenticate. |
...
| `--aws`      | Read-only bind-mount of `~/.aws/config` and `~/.aws/sso/` only. `~/.aws/credentials` (long-lived keys) and `~/.aws/cli/cache/` are **not** exposed. Host `AWS_PROFILE`/`AWS_REGION`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_SESSION_TOKEN` forwarded when set. 1Password fallback for the access-key flow alone via `CLAUDE_DOCKER_AWS_OP_REF` (`op read` on the host, canonical sub-fields `access_key_id` / `secret_access_key` / `session_token` / `region`), mirroring `--ado` / `--jira`. SSO sessions still require the host-side `aws sso login` + mounted cache; OP fallback covers static IAM-user keys (or pre-exported STS bundles), not SSO. |
...
`run.sh` calls `op read` for each canonical sub-field and forwards `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` (required), plus `AWS_SESSION_TOKEN` / `AWS_REGION` (optional) when present. Required field missing → silent no-op; host AWS env vars take precedence when both are set. Rotation point becomes the 1P item; no host file ever holds the secret.
